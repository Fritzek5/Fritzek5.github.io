...
potential_savings_str = str(int(potential_savings.round(0))) +"%"

peak_s = str(int(peak.round(0))) +"%"
trough_s = str(int(trough.round(0))) +"%"
...
# URL of the image
image_url = "https://mintensity.de/Logo_MDotB.png"
...
colors = ['brown', 'red', '#ebeb00', 'green']  # Color points for the gradient
signal_colors = np.digitize(predicted_signal, bins=[-0.5, 0.5, 1.5, 2.5], right=True)
color_mapped = []
for i in range(len(signal_colors)):
    if signal_colors[i] == 0:
        color_mapped.append(colors[0])
    elif signal_colors[i] == len(colors) + 1:
        color_mapped.append(colors[-1])
    else:
        ratio = (predicted_signal[i] - (-0.5)) / (2.5 - (-0.5))
        lower_color_index = signal_colors[i] - 1
        upper_color_index = signal_colors[i]
        lower_color = np.array(go.colors.hex_to_rgb(colors[lower_color_index])) / 255
        upper_color = np.array(go.colors.hex_to_rgb(colors[upper_color_index])) / 255
        interpolated_color = tuple((1 - ratio) * lower_color + ratio * upper_color)
        color_mapped.append(f'rgb{interpolated_color}')

# Create the base figure
fig = go.Figure()

# Add the scatter plot for renewable share with dynamic coloring based on 'predicted_signal'
for i in range(len(x_new) - 1):
    fig.add_trace(go.Scatter(
        x=[x_new[i], x_new[i+1], x_new[i+1], x_new[i]], 
        y=[predicted_share[i], predicted_share[i+1], 0, 0], 
        mode='lines',
        line=dict(color=color_mapped[i], width=3),  
        fillcolor=color_mapped[i], 
        name=None
    ))
    
# Add the original line plot
fig.add_trace(go.Scatter(
    x=x_new, y=predicted_share, mode='lines', line=dict(color='black', width=1), name='Renewable Share'
))

# Add horizontal lines for peak and trough
fig.add_trace(go.Scatter(
    x=[0, x_new[highest]], y=[ppeak, ppeak], mode='lines', line=dict(color='black', dash='dash'), name='Peak'
))
fig.add_trace(go.Scatter(
    x=[0, x_new[lowest]], y=[ptrough, ptrough], mode='lines', line=dict(color='black', dash='dash'), name='Trough'
))

# Customizing the layout
fig.update_layout(
    title=f"% Erneuerbare im deutschen Strommix LOL, {dt.strftime('%d.%m.%y')}",
    xaxis=dict(title='Uhrzeit', tickmode='array', tickvals=[0, 4, 8, 12, 16, 20, 24], ticktext=['00:00', '04:00', '08:00', '12:00', '16:00', '20:00', '24:00'], tickformat='.1f'),
    yaxis=dict(title='% Erneuerbare Energie', tickformat='.1f'),
    template='plotly_white',
    dragmode=False,
    modebar={'add': [], 'remove': ["resetaxes", 'pan', 'lasso', 'pan3d', 'orbitRotation', 'zoomOutMapbox', 'resetViewGeo', 'toImage', 'sendDataToCloud'], 'orientation': 'h'}
)

# Add annotations for the peak and trough
fig.add_annotation(x=x_new[highest], y=ppeak, text=f"Peak: {ppeak:.1f}%", showarrow=True, arrowhead=1, ax=20, ay=-30)
fig.add_annotation(x=x_new[lowest], y=ptrough, text=f"Trough: {ptrough:.1f}%", showarrow=True, arrowhead=1, ax=20, ay=30)
...
fig.write_html("renewable_energy_plot.html")
